...
test.describe("Auditoría Cliente — Autenticación y protección de rutas", () => {
  ...
  test("redirige a /admin si el usuario es Administrador", async ({ page }) => {
    await injectAuthAs(page, "Administrador", 1, "Admin", "a@a.com");
    await page.goto("/cliente");
  ...
  test("redirige a /veterinario si el usuario es Veterinario", async ({ page }) => {
    await injectAuthAs(page, "Veterinario", 2, "Vet", "v@v.com");
    await page.goto("/cliente");
  ...
  test("enlace 'Agendar cita' navega a /cliente/agendar/nueva", async ({ page }) => {
    ...
    if ((await link.count()) > 0) {
      ...
      log(
        ...
        "MEDIO",
        "Enlace faltante",
        "No se encontró enlace de agendar cita en el dashboard",
      );
    }
  });
});
...
test.describe("Auditoría Cliente — Citas (/cliente/agendar)", () => {
  ...
  test("filtros de citas funcionan (Todas / Próximas / Confirmadas / Canceladas)", async ({
    page,
  }) => {
    await page.goto("/cliente/agendar");
    await page.waitForLoadState("networkidle");
    ...
    for (const link of links) {
      ...
      if (isNaN(Number(id)) && id !== "nueva") {
        log(
          "/cliente/mascotas",
          "CRÍTICO",
          "ID hardcodeado en enlace",
          `El enlace usa ID="${id}" que no es numérico — probablemente hardcodeado`,
        );
      }
    }
  });
...
test.describe("Auditoría Cliente — Detalle mascota (/cliente/mascotas/[Id])", () => {
  ...
  test("AUDIT: la ruta con ID numérico (API) muestra la mascota o 'no encontrada'", async ({
    page,
  }) => {
    ...
    if (body.includes("ID recibido desde la URL")) {
      log(
        ...
        "Hay console.log de debug que deben eliminarse",
      );
    }
  });
...
test.describe("Auditoría Cliente — Nueva mascota (/cliente/mascotas/nueva)", () => {
  test.beforeEach(async ({ page }) => {
    await injectAuth(page, "Ana García", "ana@test.com");
  });
...
test.describe("Auditoría Cliente — Perfil (/cliente/perfil)", () => {
  test.beforeEach(async ({ page }) => {
    await injectAuth(page, "Roberto Sánchez", "roberto@test.com");
  });
  ...
  test("AUDIT: el perfil muestra datos REALES del usuario, no hardcodeados", async ({ page }) => {
    ...
    if (body.includes("Ciudad de México")) {
      log(
        ...
        "Ubicación es un placeholder estático",
      );
    }
...
test.describe("Auditoría Cliente — Configuración (/cliente/configuracion)", () => {
  test.beforeEach(async ({ page }) => {
    await injectAuth(page, "Laura Mendoza", "laura@test.com");
  });
...
test.describe("Auditoría Cliente — Datos hardcodeados globales", () => {
  test.beforeEach(async ({ page }) => {
    await injectAuth(page, "Roberto Test", "roberto@test.com");
  });
...
test.afterAll(async () => {
  ...
  console.log("\n" + "═".repeat(70));
  console.log(
    `  Total: ${issues.length} issues | Críticos: ${bySev.CRÍTICO.length} | Altos: ${bySev.ALTO.length} | Medios: ${bySev.MEDIO.length}`,
  );
  console.log("═".repeat(70) + "\n");
});
