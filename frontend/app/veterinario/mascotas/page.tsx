...
const pacientes: Paciente[] = [
  {
    ...
    ultimaAtencion: "Hoy · 09:00",
    motivo: "Consulta general",
    estado: "Por atender",
    alergias: "No registradas",
    antecedentes: "Vacunación al día. Sin cirugías previas registradas.",
    historialPdf: "Historia_clinica_Max.pdf",
  },
  {
    id: "luna",
    nombre: "Luna",
    ...
    ultimaAtencion: "Hoy · 10:30",
    motivo: "Vacunación",
    estado: "Control programado",
    alergias: "No registradas",
    antecedentes: "Aplicación de vacuna triple felina registrada.",
  },
  {
    id: "rocky",
    nombre: "Rocky",
    ...
    ultimaAtencion: "28 May 2026 · Cirugía menor",
    motivo: "Control postoperatorio",
    estado: "Tratamiento activo",
    alergias: "Sensibilidad a penicilina",
    antecedentes: "Cirugía menor realizada. Requiere seguimiento clínico.",
    historialPdf: "Historia_clinica_Rocky.pdf",
  },
  {
    id: "bella",
    nombre: "Bella",
    ...
    ultimaAtencion: "Hoy · 14:00",
    motivo: "Control postoperatorio",
    estado: "En seguimiento",
    alergias: "No registradas",
    antecedentes: "Control de evolución posterior a procedimiento quirúrgico.",
    historialPdf: "Historia_clinica_Bella.pdf",
  },
];
...
function PacientesContent() {
  ...
  const pacientesFiltrados = useMemo(() => {
    ...
    return pacientes.filter((paciente) => {
      ...
      const coincideEstado =
        filtro === "Todos" || paciente.estado === filtro;

      return coincideBusqueda && coincideEstado;
    });
  }, [busqueda, filtro]);

  const porAtender = pacientes.filter(
    (paciente) => paciente.estado === "Por atender"
  ).length;

  const atendidosHoy = pacientes.filter(
    (paciente) => paciente.estado === "Atendido hoy"
  ).length;
  ...
  const tratamientosActivos = pacientes.filter(
    (paciente) => paciente.estado === "Tratamiento activo"
  ).length;
                ...
                <div className="flex items-start gap-4">
                  <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-[18px] bg-gradient-to-br from-[#DBEAFE] to-[#EFF6FF] text-[20px] font-bold text-[#2563EB] transition duration-300 group-hover:-translate-y-1 dark:from-[#1E3A8A] dark:to-[#172554] dark:text-[#BFDBFE]">
                    ...
                    <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-white dark:border-[#0F172A] dark:bg-[#0F172A]">
                      <span
                        className={`h-2.5 w-2.5 rounded-full ${colorPuntoEstado(
                          paciente.estado
                        )}`}
                      />
                    </span>
            ...
            <div className="space-y-5 p-6">
              <div className="flex items-start gap-3 rounded-[15px] border border-[#DBEAFE] bg-[#EFF6FF] px-4 py-3 text-[13px] text-[#1D4ED8] dark:border-[#1E3A8A] dark:bg-[#172554] dark:text-[#BFDBFE]">
                ...
                <p className="leading-6">
                  Información de solo consulta. Los datos generales son
                  administrados por el propietario o el administrador.
                </p>
              </div>

              <div className="grid gap-4 sm:grid-cols-2">
                <DatoPerfil titulo="Edad" valor={pacientePerfil.edad} />
                <DatoPerfil titulo="Peso" valor={pacientePerfil.peso} />
                <DatoPerfil
                  titulo="Propietario"
                  valor={pacientePerfil.propietario}
                />
                <DatoPerfil
                  titulo="Teléfono"
                  valor={pacientePerfil.telefono}
                />
                <DatoPerfil
                  titulo="Alergias"
                  valor={pacientePerfil.alergias}
                />
                <DatoPerfil
                  titulo="Estado clínico"
                  valor={pacientePerfil.estado}
                />
              </div>
...
function TarjetaIndicador({
  ...
  const estilos = {
    blue: {
      icono:
        "bg-[#E8F0FF] text-[#2563EB] dark:bg-[#1E3A8A] dark:text-[#BFDBFE]",
      linea: "bg-[#2563EB]",
    },
    green: {
      icono:
    ...
    purple: {
      icono:
        "bg-[#EDE9FE] text-[#7C3AED] dark:bg-[#4C1D95] dark:text-[#DDD6FE]",
      linea: "bg-[#8B5CF6]",
    },
    red: {
      icono:
        "bg-[#FEE2E2] text-[#DC2626] dark:bg-[#7F1D1D] dark:text-[#FECACA]",
      linea: "bg-[#EF4444]",
    },
  };
  ...
  return (
    ...
    >
      <span
        className={`absolute left-0 top-0 h-full w-1 ${estilos[color].linea}`}
      />
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-4 text-[34px] font-bold text-[#10213A] dark:text-white">
            {valor}
          </p>
        </div>
...
function EstadoBadge({ estado }: { estado: EstadoPaciente }) {
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[12px] font-semibold ${estiloEstado(
        estado
      )}`}
    >
      <span
        className={`h-2 w-2 rounded-full ${colorPuntoEstado(estado)} ${
          estado === "Por atender" || estado === "Tratamiento activo"
            ? "animate-pulse"
            : ""
        }`}
      />
